...
class ControllerExtensionShippingecpayLogistic extends Controller 
{
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting($this->module_name, $module_settings);
			...
			$this->response->redirect($this->url->link('extension/extension', 'token=' . $token . '&type=shipping', true));
		}
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_extension'),
            		'href' => $this->url->link('extension/extension', 'token=' . $token . '&type=shipping', true)
		);
		$data['breadcrumbs'][] = array(
            		'text' => $heading_title,
            		'href' => $this->url->link($this->module_path, 'token=' . $token, true)
		);          
		...
		$data['action'] = $this->url->link($this->module_path, 'token=' . $token, true);
		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $token . '&type=shipping', true);
	...
	public function create_shipping_order() {
		...
		if (!$ecpaylogistic_query->num_rows) {
			...
			if ($orderInfo) {
				...
				$logisticSubType = explode(".", $orderInfo['shipping_code']);
				...
				$_CollectionAmount = 0;
				if (strpos($orderInfo['shipping_code'],"_collection") !== false) {
					$_IsCollection = IsCollection::YES;
					$_CollectionAmount = (int)ceil($orderInfo['total']);
				}
				...
				try {
					...
					$AL->Send = array(
						...
						'GoodsAmount' => (int)ceil($orderInfo['total']),
						...
						'ReceiverName' => $orderInfo['shipping_lastname'] . $orderInfo['shipping_firstname'],
						'ReceiverCellPhone' => $orderInfo['telephone'],
						'ReceiverEmail' => $orderInfo['email'],
						'ServerReplyURL' => $server_reply_url,
						'LogisticsC2CReplyURL' => $logistics_c2c_reply_url,
						'Remark' => 'ecpay_module_opencart',
					);
					$AL->SendExtend = array(
						'ReceiverStoreID' => $orderInfo['shipping_address_1'],
						'ReturnStoreID' => $orderInfo['shipping_address_1']
					);
	...
	public function chk_create_shipping(&$route, &$data, &$output) {
        ...
        if($create_shipping_flag) {
        	...
        	$create_shipping_order_url = $this->url->link(
                ...
                'token=' . $token . '&order_id='.$data['order_id'],
                $this->url_secure
	        );
    ...
    public function install() {
		...
		$this->model_extension_event->addEvent('ecpay_logistic_payment_method', 'catalog/model/extension/extension/getExtensions/after', 'extension/payment/ecpaylogistic/chk_payment_method');
		$this->model_extension_event->addEvent('ecpay_logistic_create_shipping', 'admin/view/sale/order_info/before', 'extension/shipping/ecpaylogistic/chk_create_shipping');
		// 物流單列印判斷
		$this->model_extension_event->addEvent('ecpay_logistic_print_shipping', 'admin/view/sale/order_info/before', 'extension/shipping/ecpaylogistic/chk_print_shipping');
		$this->model_extension_event->addEvent('ecpay_logistic_javascript', 'admin/view/common/header/before', 'extension/shipping/ecpaylogistic/add_javascript');
	...
	public function uninstall() {
		...
		// delete event
		$this->load->model('extension/event');
		$this->model_extension_event->deleteEvent('ecpay_logistic_payment_method');
		$this->model_extension_event->deleteEvent('ecpay_logistic_create_shipping');
		$this->model_extension_event->deleteEvent('ecpay_logistic_print_shipping');
		$this->model_extension_event->deleteEvent('ecpay_logistic_javascript');	
	}	
}
